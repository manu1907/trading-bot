...
remain available after restart. Dashboards, deployment-specific external alert
routing, and a production-grade indexed audit backend are still future work. Audit store
...
`ops/prometheus/pause-governance-alerts.yml`; deployment-specific routing still
has to map rule labels such as `severity` and `routing_hint` to actual
