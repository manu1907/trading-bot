...
public final class InterventionAcknowledgementService {
    ...
    public CompletableFuture<PublishedTradingEvent> acknowledgePosition(
            PositionInterventionAcknowledgementRequest request
    ) {
        Objects.requireNonNull(request, "request");
        String provider = requireText(request.provider(), "provider");
        String environment = requireText(request.environment(), "environment");
        ...
        String market = requireText(request.market(), "market");
        String symbol = requireText(request.symbol(), "symbol");
        String positionSide = requireText(request.positionSide(), "positionSide");
        String interventionReason = requireText(request.interventionReason(), "interventionReason");
        requireMatchingProjectedPositionIntervention(
                provider,
                environment,
                account,
                market,
        ...
        InterventionAcknowledgementEvent event = InterventionAcknowledgementEvent.newBuilder()
                ...
                .setAcknowledgementReason(requireText(request.acknowledgementReason(), "acknowledgementReason"))
                .setAcknowledgedAtMicros(Instant.now(clock))
                .setAttributes(positionAttributes(positionSide, request.attributes()))
                .build();
        return eventBus.publish(envelope(event));
    }
    ...
    private void requireMatchingProjectedPositionIntervention(
            String provider,
            String environment,
            String account,
            String market,
        ...
        TradingStateProjection.PositionState position = projection.position(
                ...
                .orElseThrow(() -> new IllegalStateException("No projected position exists for acknowledgement"));
        ...
        if (!interventionReason.equals(position.interventionReason())) {
            throw new IllegalStateException("Projected position intervention reason does not match acknowledgement");
        }
    }
    ...
    private Map<CharSequence, CharSequence> positionAttributes(
            String positionSide,
            Map<CharSequence, CharSequence> attributes
    ) {
        Map<CharSequence, CharSequence> enriched = new LinkedHashMap<>();
        if (attributes != null) {
            enriched.putAll(attributes);
        }
        enriched.put("position_side", positionSide);
    ...
    public record PositionInterventionAcknowledgementRequest(
            String provider,
            String environment,
            String account,
            String market,
        ...
        public PositionInterventionAcknowledgementRequest {
            attributes = attributes == null ? Map.of() : Map.copyOf(attributes);
        }
    }
}
